    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<UserDetailDto?> GetUserByIdAsync(int id)
        {
            ...
            return new UserDetailDto
            {
                ...
                ProdutorIds = usuario.ProdutoresGerenciados
        ...
        public async Task<bool> RegistrarAsync(RegisterDto registerDto)
        {
            ...
            await _auditoriaService.RegistrarLog(
                "CRIACAO_USUARIO",
                $"Utilizador '{novoUsuario.Email}' (Código: {novoUsuario.Codigo}) foi criado."
            );

            // Associação Financeiro → Produtores (somente quando PerfilId = 3)
            if (registerDto.PerfilId == 3 && registerDto.ProdutorIds != null && registerDto.ProdutorIds.Any())
            {
                ...
                {
                    _context.FinanceiroProdutor.Add(new FinanceiroProdutor
                    {
                        FinanceiroId = novoUsuario.Id,
                ...
                await _auditoriaService.RegistrarLog(
                    "ASSOCIACAO_USUARIO",
                    $"Utilizador Financeiro '{novoUsuario.Email}' associado aos Produtores IDs: {string.Join(", ", registerDto.ProdutorIds)}."
                );
            }
        ...
        public async Task<bool> UpdateUserAsync(int id, UpdateUserDto updateUserDto)
        {
            ...
            // Impede email duplicado em outro usuário
            if (await _context.Usuarios.AnyAsync(u => u.Email == updateUserDto.Email && u.Id != id))
                return false;
            ...
            // Recria associações se for Financeiro
            usuario.ProdutoresGerenciados.Clear();
            if (usuario.PerfilId == 3 && updateUserDto.ProdutorIds != null && updateUserDto.ProdutorIds.Any())
            {
                foreach (var produtorId in updateUserDto.ProdutorIds)
                    usuario.ProdutoresGerenciados.Add(new FinanceiroProdutor { ProdutorId = produtorId });
            }
            ...
            await _auditoriaService.RegistrarLog(
                "ATUALIZACAO_USUARIO",
                $"Utilizador '{usuario.Email}' (ID: {usuario.Id}) foi atualizado."
            );
        ...
        public async Task<bool> DeleteUserAsync(int id)
        {
            ...
            {
                if (await _context.Granjas.AnyAsync(g => g.UsuarioId == id))
                    throw new InvalidOperationException("Este produtor possui granjas e/ou lotes associados e não pode ser excluído.");

                if (await _context.FinanceiroProdutor.AnyAsync(fp => fp.ProdutorId == id))
                    throw new InvalidOperationException("Este produtor está associado a um ou mais utilizadores financeiros e não pode ser excluído.");
            }
            ...
            await _auditoriaService.RegistrarLog(
                "DELECAO_USUARIO",
                $"Utilizador '{nomeUsuarioDeletado}' (ID: {id}) foi deletado."
            );
        ...
        public async Task<bool> UpdateProfileAsync(UpdateProfileDto profileDto)
        {
            ...
            await _auditoriaService.RegistrarLog(
                "ATUALIZACAO_PERFIL",
                $"Utilizador (ID: {userId}) atualizou o próprio perfil."
            );
        ...
        public async Task<bool> ChangePasswordAsync(ChangePasswordDto passwordDto)
        {
            ...
            await _auditoriaService.RegistrarLog(
                "TROCA_SENHA",
                $"Utilizador (ID: {userId}) alterou a própria senha."
            );
